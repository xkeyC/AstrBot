...
class TestBuildMainAgent:
    ...
    async def test_build_main_agent_applies_selected_model_to_existing_request(
        ...
        assert result is not None
        # The request is a working copy of the plugin-provided one, so the
        # plugin keeps its own attachments and model choice untouched.
        assert result.provider_request is not existing_req
        assert result.provider_request.prompt == "Existing prompt"
        assert result.provider_request.model == "override-model"
        assert existing_req.model is None
