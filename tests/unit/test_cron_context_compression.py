...
async def test_wakeup_uses_same_compression_settings_as_chat(
    ...
    # Codex is the only selectable runner here, so pin the legacy local runner
    # that the internal stage still reads its compression settings from.
    conf["agent_runner"] = {
        "runner_type": "local",
    ...
    # This fork reads the configured runner type first so Codex sessions can be
    # dispatched to their own wakeup, so the config is read more than once. The
    # agent settings must still be resolved from the bound session.
    ctx.get_config.assert_called_with(umo="test:FriendMessage:user123")
