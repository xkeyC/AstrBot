...
from astrbot.core.config import agent_runner as agent_runner_config
...
def harness(tmp_path, monkeypatch):
    ...
    # These upstream tests drive the local agent stage. Codex is the only
    # selectable runner in this fork and `local` would be migrated away, so
    # make it selectable for the duration of the test and pick it; otherwise
    # the stage would start a real Codex engine against the real codex_home.
    monkeypatch.setattr(
        agent_runner_config, "SELECTABLE_AGENT_RUNNER_TYPES", ("codex", "local")
    )
    config["agent_runner"] = {
        "runner_type": "local",
        "config": copy.deepcopy(
            agent_runner_config.AGENT_RUNNER_CONFIG_DEFAULTS["local"]
        ),
    }
